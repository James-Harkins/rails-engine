...
describe Invoice, type: :model do
  ...
  describe "instance methods" do
    describe ".invoice_items_only_contain_item" do
      it "returns true if all of the invoice's invoice_items contain only the item passed in as an argument" do
        merchant_1 = create(:merchant)
        ...
        expect(invoice_1.invoice_items_only_contain_item(item_1.id)).to eq(false)
        expect(invoice_2.invoice_items_only_contain_item(item_1.id)).to eq(false)
        expect(invoice_2.invoice_items_only_contain_item(item_4.id)).to eq(true)
        expect(invoice_3.invoice_items_only_contain_item(item_4.id)).to eq(false)
        expect(invoice_4.invoice_items_only_contain_item(item_1.id)).to eq(true)
        expect(invoice_4.invoice_items_only_contain_item(item_2.id)).to eq(false)
        expect(invoice_5.invoice_items_only_contain_item(item_2.id)).to eq(false)
        expect(invoice_5.invoice_items_only_contain_item(item_4.id)).to eq(true)
      end
    end
  end
end
